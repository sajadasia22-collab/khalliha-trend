...
المراحل 0 إلى 14 مكتملة، والإصدار v4 منشور فعلياً على [khalliha-trend.vercel.app](https://khalliha-trend.vercel.app). المشروع مستودع Git مربوط بـGitHub، وVercel ينشر فرع `main` تلقائياً، وقاعدة الإنتاج PostgreSQL على Supabase مع migrations مطبّقة عبر خط النشر.

يشمل الإصدار الحالي المصادقة وRBAC، الحملات ودورة المراجعة، الانضمام والإرسالات، الإحصائيات والأرباح، Ledger والمحافظ والسحوبات اليدوية، مكافحة الاحتيال والنزاعات، الملفات المهنية ومعرض الأعمال ودليل الصناع، المتابعة والمجتمع، مراسلة الحملات مع البحث والحظر والبلاغات، مركز إعدادات متجاوب، إشعارات قابلة للتخصيص، وتحليلات لكل دور. التكاملات الحقيقية للدفع والسوشال ميديا لم تبدأ لأنها تتطلب حسابات مزودين وCredentials وSandbox وموافقة صريحة.
...
- تحقق threat model موثّق في `docs/THREAT_MODEL.md` §8؛ أُغلقت فجوات `AuditLog` وrate limiting وCSP واختبارات رفع الملفات.
- `pnpm audit` أصبح نظيفاً (ثغرتان متوسطتان transitive تم تثبيتهما عبر `pnpm.overrides`).

## ما تم تنفيذه في المرحلة 13
...
- `prisma migrate status` تؤكد عدم وجود drift، و`vercel.json` يطبق migrations قبل البناء.
- ربط GitHub بـVercel ونشر الإنتاج على `khalliha-trend.vercel.app` مع قاعدة Supabase وفحوصات health/readiness.
- مراقبة أخطاء منظمة ونسخ احتياطي موثق، مع تنبيه webhook اختياري يحتاج ضبطاً يدوياً في Vercel.

## ما تم تنفيذه في المرحلة 14

- ملف مهني عام لصانع المحتوى، معرض أعمال، دليل بحث، ومتابعة.
- مجتمع نصوص وصور وروابط مع الإعجاب والتعليقات والحفظ والمشاركة والحظر والكتم والبلاغات والإشراف.
- مراسلة مرتبطة بالحملات بين العلامة والصانع المنضم، مع البحث والمقروء/غير المقروء والخصوصية والحظر والبلاغات وطابور مراجعة إداري.
- مركز إعدادات قابل للبحث مع تفضيلات الإشعارات والخصوصية والمظهر وتقليل الحركة وتصدير البيانات.
- تنقل هاتف بأربع وجهات أساسية ولوحة «المزيد»، واختبارات آلية لكل روابط الأدوار وعدم التمدد الأفقي.

## المرحلة التالية

1. تفعيل `ERROR_ALERT_WEBHOOK_URL` في Vercel إذا كانت تنبيهات Slack/Discord مطلوبة.
2. تشغيل تجريبي بمستخدمين حقيقيين وجمع الملاحظات.
3. بدء تكاملات الدفع والسوشال الرسمية بعد توفير الحسابات والـCredentials والـSandbox والموافقة الصريحة.
