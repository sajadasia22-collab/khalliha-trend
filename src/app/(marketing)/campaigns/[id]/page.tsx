      ...
      <main
        className={`bg-[var(--color-bg)] text-[var(--color-text)] dir-rtl ${
          dashboardRole
            ...
            : "mx-auto max-w-4xl px-5 py-12 lg:px-8"
        }`}
      >
        {dashboardRole && (
          <DashboardHeader dashboardRole={dashboardRole} userLabel={userLabel} />
        )}
        <div className={dashboardRole ? "mx-auto max-w-4xl px-5 py-12 lg:px-8" : ""}>
  ...
  return (
    ...
    >
      ...
      <div className={dashboardRole ? "mx-auto max-w-4xl px-5 py-12 lg:px-8" : ""}>
        ...
        <p className="mt-6 leading-relaxed text-[var(--color-text)]">
          {campaign.summary}
        </p>
        ...
        {user ? (
          user.role === "CREATOR" ? (
            ...
            <div className="mt-10 card border border-[var(--color-border)] bg-[var(--color-surface)] p-8 text-center text-sm font-bold text-[var(--color-text-secondary)]">
              أنت مسجل كـ {user.role === "BRAND" ? "علامة تجارية" : "مسؤول نظام"}. فقط
              صناع المحتوى يمكنهم الانضمام وتقديم منشورات للحملات.
            </div>
          )
        ) : (
