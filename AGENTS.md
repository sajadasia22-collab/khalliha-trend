...
المرحلة الحالية: المرحلة 12 مكتملة، والمرحلة 13 جزئية عمداً (بانتظار شراء اشتراك سيرفر من صاحب المنتج).

المراحل 1 إلى 12 مكتملة حسب معايير القبول في [ROADMAP.md](docs/ROADMAP.md). توجد قاعدة بيانات PostgreSQL محلية فعلية تعمل.

**تنبيه أمني مهم من المرحلة 12**: تم اكتشاف وإصلاح ثغرة سباق مالي حقيقية (double-spend) في مسارات السحب وحجز ميزانية الحملة — كانت تفحص الرصيد بلا قفل صف Postgres. الإصلاح في `LedgerEngine.lockFinancialAccount`. راجع `docs/THREAT_MODEL.md` §8 لفجوات أمنية حقيقية متبقية (لا AuditLog فعلي، لا rate limiting، لا CSP) قبل أي تفكير بالنشر الفعلي.

**تنبيه نشر من المرحلة 13**: `.github/workflows/ci.yml` أصبح خط أنابيب فعلي (Postgres service، migrations، اختبارات حقيقية، E2E، dependency audit) وتم التحقق منه محلياً. لكن **المشروع ليس git repository بعد** — لا `.git` إطلاقاً، وهذا يمنع أي CI حقيقي على GitHub أو أي نشر (Vercel وغيره) حتى تتم تهيئته. راجع `docs/DEPLOYMENT.md` §8 لتقرير جاهزية الإنتاج الكامل.
...
- نموذج `AuditLog` فعلي في قاعدة البيانات (موثّق كمتطلب في SECURITY.md لكن غير مُنفَّذ).
- Rate limiting على أي endpoint.
- Security headers / CSP في `next.config.ts`.
- التكاملات الحقيقية (دفع وسوشال ميديا).
- **المشروع ليس git repository بعد** — عائق أساسي أمام أي نشر أو CI حقيقي على GitHub.
- استضافة فعلية، monitoring، backup — مؤجّلة بقرار صاحب المنتج لحين شراء اشتراك سيرفر.

## المرحلة التالية

إكمال المرحلة 13 (استضافة فعلية) بعد شراء اشتراك سيرفر.
