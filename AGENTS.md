...
المرحلة الحالية: المراحل 0–14 مكتملة، والمنصة منشورة على Vercel. الإصدار v4 يضيف مراسلة الحملات ومركز الإعدادات وتجربة الهاتف المحسنة.

المراحل 0 إلى 14 مكتملة حسب [ROADMAP.md](docs/ROADMAP.md). توجد قاعدة PostgreSQL محلية وقاعدة إنتاج Supabase، والمشروع Git repository مربوط بـGitHub.
...
- نماذج قرارات إدارية RTL متجاوبة داخل الصفحة بدل `window.prompt`، مع أسباب إلزامية للرفض والتعديلات والقرارات المالية.
- أصول الحملات عبر روابط مشاركة فعلية فقط (Google Drive/Dropbox/OneDrive)؛ أُزيل الرفع التجريبي غير المربوط بتخزين.
...
- مراسلة حملات بين العلامة وصانع منضم فعلياً، مع بحث ومقروء/غير مقروء وحظر وإبلاغ وطابور مراجعة إداري.
- مركز إعدادات قابل للبحث، مظهر فاتح/داكن، عرض مدمج، تقليل الحركة، وتصدير خاص لبيانات المستخدم.
- تنقل هاتف من أربع وجهات أساسية ولوحة «المزيد»، وتحسين الملف العام وصفحات تحرير ملفات الصانع والعلامة.

## الميزات غير المكتملة

- التكاملات الحقيقية (دفع وسوشال ميديا).
- تفعيل `ERROR_ALERT_WEBHOOK_URL` يدوياً في Vercel للتنبيه الخارجي.
- التشغيل التجريبي مع مستخدمين حقيقيين وجمع ملاحظات الاستخدام لتحديد أولويات الإصدار التالي.

## المرحلة التالية

تفعيل تنبيه الأخطاء الخارجي، ثم بدء تشغيل تجريبي بمستخدمين حقيقيين. التكاملات الرسمية للدفع والسوشال تبدأ فقط بعد توفير Credentials وSandbox وموافقة صريحة.
