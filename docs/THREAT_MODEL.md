...
**فجوات حقيقية متبقية (لم تُغلق في هذه المرحلة):**

- **لا يوجد نموذج `AuditLog` فعلي في قاعدة البيانات** رغم أن هذا المستند وَ`SECURITY.md` يفترضان وجوده كمتطلب أساسي ("Audit logs تعمل" في checklist ما قبل الإنتاج). الإجراءات الإدارية الحساسة تُسجَّل حالياً بشكل جزئي فقط (`reviewedByUserId`/`reviewedAt` على السجلات نفسها)، بلا سجل مركزي بـ before/after. يحتاج تصميم/migration منفصل — خارج نطاق هذه المرحلة (اختبارات)، ويُرشَّح للمرحلة 13 أو تكليف مخصص.
- **لا rate limiting** على أي endpoint (تسجيل الدخول، التسجيل، إلخ) — القسم 2 من `SECURITY.md` يطلبه صراحة.
- **لا Security headers/CSP** في `next.config.ts` (لا `Content-Security-Policy`، لا `X-Frame-Options`، إلخ).
- **Upload validation tests**: لا ينطبق حالياً — لا توجد أي ميزة رفع ملفات في الكود (الأصول والصور كلها روابط خارجية)، فالبند غير قابل للاختبار إلى أن تُضاف ميزة رفع فعلية.
- **Admin audit log tests**: غير ممكنة حالياً لعدم وجود النموذج (انظر أعلاه).

هذه الفجوات موثّقة هنا بدل إخفائها؛ إغلاقها يتطلب قرارات تصميم (شكل AuditLog، استراتيجية rate limiting) تفوق نطاق "كتابة اختبارات" في هذه المرحلة.
