...
## وين وصلنا — ملخص الحالة (آخر تحديث: 2026-07-15)

**كل المراحل 0–14 مكتملة، والإصدار v4 منشور ومتحقق منه على الإنتاج** (`khalliha-trend.vercel.app`). آخر ما أُنجز:
...
- **أدلة النزاعات (2026-07-15)**: رفع صور/PDF على النزاعات بتحقق magic bytes وحدود صارمة وتخزين داخل قاعدة البيانات، مع تصريح تنزيل مقصور على الأطراف والمدراء — `CHANGELOG.md` 2026-07-15.
- **مراسلة الحملات (2026-07-15)**: محادثات بين العلامة وصانع منضم للحملة فقط، مع بحث، مقروء/غير مقروء، إشعارات، خصوصية، حظر، بلاغات، وطابور مراجعة إداري.
- **إصدار v4 (2026-07-15)**: اكتمل CI بنجاح ونُشر الإصدار على Vercel مع تطبيق migration المراسلة، ثم نجحت فحوصات الصحة والجاهزية على الإنتاج.
- **تدقيق الواجهة بعد الإصدار**: فُحصت جميع وجهات قوائم الأدوار على الهاتف، وأُغلقت آخر نوافذ `prompt` وخيار رفع ملفات الحملة الوهمي.
- **صيانة البنية**: نُقلت حماية المسارات إلى اصطلاح Next.js `proxy.ts` الحديث، وحُدّثت إجراءات CI للعمل على Node.js 24 بلا تحذيرات مهملة.

**التالي عند الاستئناف (بالأولوية):**

1. **المرحلة 14 مكتملة**: اكتمل الملف المهني، المعرض، الدليل، المتابعة، المجتمع، ومراسلة الحملات مع الإشراف.
2. **خطوة يدوية على صاحب المشروع**: إنشاء Incoming Webhook (Slack/Discord) وإضافته كـ`ERROR_ALERT_WEBHOOK_URL` في Vercel لتفعيل تنبيهات الأخطاء.
3. **تكامل الدفع الحقيقي** (ZainCash/FastPay): يبدأ بالحصول على حساب تاجر + credentials + sandbox ومراجعة قانونية — البنية جاهزة (`PAYMENT_INTEGRATIONS.md`).
4. **تكامل السوشال الرسمي** (TikTok/Meta/YouTube APIs): يبدأ بالتقديم على حسابات المطورين وانتظار الموافقات (`SOCIAL_INTEGRATIONS.md`).
5. تشغيل فعلي بمستخدمين حقيقيين — ملاحظات الاستخدام سترسم الأولويات القادمة.
...
## المرحلة 14: الهوية المهنية والمجتمع - مكتملة
...
الدفعة الخامسة المكتملة:

- مراسلة مرتبطة بالحملة، مقصورة على العلامة المالكة وصانع محتوى منضم فعلياً.
- بحث في قائمة المحادثات وداخل الرسائل، مع حالة مقروء وعدّاد غير مقروء وإشعارات.
- تطبيق إعداد خصوصية بدء الرسائل والحظر المتبادل على السيرفر.
- إبلاغ عن رسالة بعينها وطابور إدارة مع إزالة الرسالة وأثر تدقيق.
