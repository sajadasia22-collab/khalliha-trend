...
1. **خطوة يدوية على صاحب المشروع**: إنشاء Incoming Webhook (Slack/Discord) وإضافته كـ`ERROR_ALERT_WEBHOOK_URL` في Vercel لتفعيل تنبيهات الأخطاء.
2. **تكامل الدفع الحقيقي** (ZainCash/FastPay): يبدأ بالحصول على حساب تاجر + credentials + sandbox ومراجعة قانونية — البنية جاهزة (`PAYMENT_INTEGRATIONS.md`).
3. **تكامل السوشال الرسمي** (TikTok/Meta/YouTube APIs): يبدأ بالتقديم على حسابات المطورين وانتظار الموافقات (`SOCIAL_INTEGRATIONS.md`).
4. ميزات أكبر لاحقاً: رفع ملفات (أدلة النزاعات — يحتاج قرار تخزين مثل Supabase Storage)، وتطبيق موبايل (`MOBILE_ROADMAP.md`).
